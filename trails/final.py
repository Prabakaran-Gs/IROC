...
class Autonomous:
    def __init__(self):
        ...
        self.ser2 = serial.Serial(port='/dev/ttyUSB0',baudrate=9600,timeout=1)
    ...
    def serial_read(self):
        ...
        while True:
            try:
                ...
                if len(data) == 2:
                    self.isborder,ispicked = list(map(int,data))
                time.sleep(0.1)
        ...
        while not self.ispicked:
            ...
            if len(result[0]):
                ...
                if self.point_within(point_obj,self.polygon_2):
                    ...
                    #text  += f'rotate angle{angle:.2f} and pick'
                    ...
                    if x>=0 and x <= 150:
                        ...
                        #logger.log_info("right hivision test tube")
            if not flag:
                self.ser.write('f'.encode())
            #logger.save_image(self.hivision_image, f"test_tube_hivision{ctr}.jpg")
            ctr += 1
            try:
                cv2.imshow("test_tube",self.hivision_image)
            except:
                print("Error in displaying image hik vision")
            if cv2.waitKey(1) & 0XFF == ord('q'):
                    break

    def sample_cantainer(self):
        print("Initializing Sample Droping")
        #logger.log_info("Initializing Sample Droping")
        cap = cv2.VideoCapture(0)
        ctr = 0
        if not cap.isOpened():
            print("Error: Could not open camera.")
            #logger.log_error("Error: Could not open camera. sample container code")
            exit()
        ...
        while self.ispicked:
            ...
            if not ret:
                print("Error: Could not read frame.")
                #logger.log_error("Error: Could not read frame. sample container code")
            ...
            if len(result[0]) == 1:
                ...
                if self.point_within(point_obj,self.polygon_2):
                    ...
                    print('sample test tube droped successfully')
                    back_time = time.time()
                    while True:
                        self.ser.write('b'.encode())
                        time.sleep(0.1)
                    ...
                    if x>=0 and x <= 230:
                        self.ser.write('l'.encode())
                        print("left hivision sample container")
                    elif x>=441 and x <=1920:
                        print("right hivision sample container")
                        self.ser.write('r'.encode())
                
                cv2.imshow("Sample Container",self.hivision_image)
                if cv2.waitKey(1) & 0XFF == ord('q'):
                    break
                ctr += 1
                #logger.save_image(self.hivision_image,f"sample_container{ctr}.jpg")
    ...
    def start_all(self):
        ...
        while not self.stop_signal:
            ...
            if err == sl.ERROR_CODE.SUCCESS:
                ...
                self.processed_image = result[0].plot(labels=False) #labels
                
                if self.isborder:
                    print("border ",self.isborder)
                    start_time = time.time()
                    if self.prevState == 1: # right turn
                        while self.isborder:  
                            self.ser.write('r'.encode())
                    ...
                    self.prevState = 1 - self.prevState
                ...
                for obj in result[0].boxes:
                    if obj.cls == 0:
                        ...
                        if self.within_poly(obj,self.polygon):
                            ...
                            cv2.putText(self.processed_image, text, text_position, self.font, self.font_scale, self.font_color, self.thickness)
                        
                    elif obj.cls == 1:
                        print("crater")
                        if self.within_poly(obj,self.polygon):
                                print("crater within range")
                                (x,y) = self.get_pos(obj)
                                depth_value = self.get_depth(x,y)
                                text = f"crater "
                                self.to_avoid = True
                ...
                if self.to_avoid:
                    if self.prevState == 1:
                        self.ser.write('r'.encode())
                        print("obstance avoid right")
                    else:
                        self.ser.write('l'.encode())
                        print("obstance avoid left")
                    time.sleep(0.1)
                    self.ser.write('f'.encode())
                else:
                    for obj in result[0].boxes:
                        print("picked status ",self.ispicked)
                        if not self.ispicked:
                            if obj.cls == 2:
                                    print("sample test tube detected")
                                    ...
                                    time.sleep(0.1)
                                    self.ser.write('f'.encode())
                                    print("forward test tube")
                                    x1, y1, x2, y2 = box
                                    text_position = ((x1 + x2 - text_size[0]) // 2, y1 - 10)
                                    cv2.putText(self.processed_image, text, text_position, self.font, self.font_scale, self.font_color, self.thickness)
                            else:
                                print("sample test tube not deteched forward")
                                self.ser.write('f'.encode())
                                time.sleep(0.1)
                        elif self.ispicked:
                            if obj.cls == 3:
                                    ...
                                    if self.within_poly(obj,self.polygon_1):
                                        ...
                                        if depth_value:
                                            ...
                                            if (depth_value//10) <= 70:
                                                ...
                                                print("Autonomous completed ")
                                                exit(0)
                                                print("finished Hik Vision")
                                            else:
                                                self.ser.write('f'.encode())
                                                print("sample container out of range forward")
                                        else:
                                            print("sample container depth not found")
                                            self.ser.write('f'.encode())
                                    else:
                                        if x>=0 and x <= 888:
                                            self.ser.write('l'.encode())
                                ...
                                print("forward no sample container")
                                self.ser.write('f'.encode())
                        else:
                            self.ser.write('f'.encode())
                            print("foward")
            ...
            self.processed_image = self.draw_poly(self.processed_image,self.polygon_coords)
            self.processed_image = self.draw_poly(self.processed_image,self.polygon_coords_1)
            self.processed_image = cv2.resize(self.processed_image,(1024,600))
